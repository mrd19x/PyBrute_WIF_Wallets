...
def generate_private_key(seed=None):
    if seed:
        return seed.to_bytes(32, byteorder='big')
...
def match_addresses(addresses, match_file, log_file, process_id, stats_queue, start_range, end_range):
    count = 0
    start_time = time.time()
    
    for seed in range(start_range, end_range + 1):
        try:
            private_key_bytes = generate_private_key(seed)
...
def check_addresses_from_file(data_file, match_file, log_file, num_processes, start_range, end_range):
    try:
    ...
    for i in range(num_processes):
        p = multiprocessing.Process(target=match_addresses, args=(addresses, f"/app/results/{match_file}", log_file, i+1, stats_queue, start_range, end_range))
        p.start()
        processes.append(p)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='BTC Address Matcher with range of private keys.')
    parser.add_argument('--start', type=int, required=True, help='Start of the range for private keys.')
    parser.add_argument('--end', type=int, required=True, help='End of the range for private keys.')
    ...
    check_addresses_from_file(data_file, match_file, log_file, num_processes, start_range, end_range)
