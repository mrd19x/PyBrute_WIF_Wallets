...
# Generate a random private key within a range
def generate_private_key(start_range, end_range):
    return int.to_bytes(random.randint(start_range, end_range), 32, byteorder='big')
...
def match_addresses(addresses, match_file, log_file_compressed, log_file_uncompressed, process_id, stats_queue, start_range, end_range):
    count = 0
    ...
    while True:
        try:
            ...
            # Write each generated key to the corresponding log file
            with open(f"{log_file_uncompressed}_{process_id}.log", 'a') as log_uncompressed:
                log_uncompressed.write(f"{private_key_bytes.hex()}|{wif_uncompressed}|{uncompressed_address}\n")
            with open(f"{log_file_compressed}_{process_id}.log", 'a') as log_compressed:
                log_compressed.write(f"{private_key_bytes.hex()}|{wif_compressed}|{compressed_address}\n")
...
def check_addresses_from_file(data_file, match_file, log_file_compressed, log_file_uncompressed, num_processes, start_range, end_range):
    ...
    for i in range(num_processes):
        ...
        if i == num_processes - 1:  # Last process gets the remainder
            proc_end_range = end_range
        p = multiprocessing.Process(target=match_addresses, args=(addresses, match_file, log_file_compressed, log_file_uncompressed, i+1, stats_queue, proc_start_range, proc_end_range))
        p.start()
        processes.append(p)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start', type=str, required=True, help='Start of the keyspace range in hexadecimal (e.g., 0x20000000000000000)')
    parser.add_argument('--end', type=str, required=True, help='End of the keyspace range in hexadecimal (e.g., 0x3ffffffffffffffff)')
    parser.add_argument('--num_processes', type=int, required=True, help='Number of processes to use')
    args = parser.parse_args()

    # Convert hexadecimal arguments to integers
    start_range = int(args.start, 16)
    end_range = int(args.end, 16)

    data_file = 'data.txt'
    match_file = 'match.txt'
    log_file_compressed = 'process_log_compressed'  # Base name for compressed log files
    log_file_uncompressed = 'process_log_uncompressed'  # Base name for uncompressed log files
    num_processes = args.num_processes  # Set the number of processes (cores) you want to use
