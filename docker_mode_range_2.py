...
def match_addresses_cpu(addresses, match_file, log_file_compressed, log_file_uncompressed, process_id, stats_queue, start_range, end_range):
    count = 0
    ...
    while True:
        try:
            ...
            log_uncompressed_buffer.append(f"{wif_uncompressed}|{uncompressed_address}\n")
            log_compressed_buffer.append(f"{wif_compressed}|{compressed_address}\n")

            # Write buffers to file when buffer reaches buffer_size
            if len(log_uncompressed_buffer) >= buffer_size:
                with open(f"{log_file_uncompressed}_{process_id}.log", 'a') as log_uncompressed:
                    log_uncompressed.writelines(log_uncompressed_buffer)
                log_uncompressed_buffer.clear()

            if len(log_compressed_buffer) >= buffer_size:
                with open(f"{log_file_compressed}_{process_id}.log", 'a') as log_compressed:
                    log_compressed.writelines(log_compressed_buffer)
                log_compressed_buffer.clear()
    ...
    # Write remaining buffer contents to file
    if len(log_uncompressed_buffer) > 0:
        with open(f"{log_file_uncompressed}_{process_id}.log", 'a') as log_uncompressed:
            log_uncompressed.writelines(log_uncompressed_buffer)

    if len(log_compressed_buffer) > 0:
        with open(f"{log_file_compressed}_{process_id}.log", 'a') as log_compressed:
            log_compressed.writelines(log_compressed_buffer)

# Parsing arguments, multiprocessing, etc, would follow here.
...
def check_addresses_from_file(data_file, match_file, log_file_compressed, log_file_uncompressed, num_processes, start_range, end_range):
    ...
    if start_range is not None and end_range is not None:
        ...
        for i in range(num_processes):
            ...
            p = multiprocessing.Process(target=match_addresses_cpu, args=(addresses, match_file, log_file_compressed, log_file_uncompressed, i+1, stats_queue, proc_start_range, proc_end_range))
            p.start()
            processes.append(p)
    else:
        for i in range(num_processes):
            p = multiprocessing.Process(target=match_addresses_cpu, args=(addresses, match_file, log_file_compressed, log_file_uncompressed, i+1, stats_queue, 0, SECP256k1_ORDER))
            p.start()
            processes.append(p)
